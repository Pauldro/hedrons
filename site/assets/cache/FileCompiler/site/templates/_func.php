...
function renderNav(\ProcessWire\PageArray $items) {
	...
	echo "<ul class='nav list-unstyled' role='navigation'>";
...
function renderNavTree($items, $maxDepth = 3) {
	...
	foreach($items as $item) {
		...
		if($item->id == \ProcessWire\wire('page')->id) {
			echo "<li class='active' aria-current='true'><span class='visually-hidden'>Current page: </span>";
		} else {
			echo "<li>";
		}
...
function rendersitemap($items, $maxDepth = 3) {
	$count = 1;
	// if we've been given just one item, convert it to an array of items
	//if ($items instanceof \ProcessWire\Page)
	//$items = array($items);
	$items = $items->getArray();
	// if there aren't any items to output, exit now
	if (empty($items)) return;

	foreach (array_chunk($items, 3, true) as $itemarray) {
		echo "<div class='row'>";
		foreach ($itemarray as $item) {
			if ($item->showinsitemap) {
				echo '<div class="col-sm-4 formgroup">';
